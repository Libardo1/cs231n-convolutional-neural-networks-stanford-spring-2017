...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    log_probabilities = -np.log(probabilities[np.arange(N), y])
    data_loss = np.sum(log_probabilities) / N
    regulator_loss = reg*np.sum(W1 * W1) + reg*np.sum(W2 * W2)
    loss = data_loss + regulator_loss
