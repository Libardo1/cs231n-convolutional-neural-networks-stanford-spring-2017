...
def batchnorm_backward(dout, cache):
    ...
    # The gradient computations are also listed in Ioffe & Szegedy [2015]. See 
    # the course notes.
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # We kill inacitve neurons by masking them out.
        dx = dout * mask
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    H_filter = 1 + int((H + 2*pad - HH) / stride)
    W_filter = 1 + int((W + 2*pad - WW) / stride)
        ...
        for z in range(F):
            for j in range(H_filter):
                for k in range(W_filter):
                    out[i, z, j, k] = np.sum(
                        x[i,:,j*stride:(j*stride+HH), k*stride:(k*stride+WW)] * w[z,:,:,:]
                    ) + b[z]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
